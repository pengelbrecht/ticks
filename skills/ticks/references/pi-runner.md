...
Model specs use `[provider/]model[:thinking]` in the environment overrides; in `runners.toml` the same choice is `model` plus `effort`, because the `Model` pattern rejects `:`. For Codex subscription/OAuth credentials use `openai-codex/<model>`, not the API-key `openai/<model>`. Ordinary implementation routes among economy/balanced/strong from explicit metadata and conservative task shape. Review and closeout have dedicated frontier process tiers, and this adapter never defaults them: `[roles.plan]`, `[roles.scout]`, `[roles.review]` and `[roles.closeout]` are read only from their own explicit tables and never resolve against `[roles.implement]`, so a config that omits `[roles.review]` blocks the final review instead of running it on the model that wrote the code. Closeout has the one fallback — the planner model, when `[roles.closeout]` is absent — and blocks when neither is configured. The herdr-substrate spawner's unlisted-role fallback to `implement` (a rule the old `tk herd spawn` enforced and `ticfac run-epic` now does) is a spawner rule, not a gate rule.
