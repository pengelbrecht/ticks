...
A human tick surviving into the final graph should have a reason it couldn't be resolved during planning. Resolve is the default; awaiting is the exception you justify — and the justification has a mechanical form: a `gate:` line in the tick's description or a note (`tk note <id> "gate: <why planning could not settle this>"`). `tk graph` warns on every open gated tick without one and lists them as `unjustified_gates` in `--json`; a warning, never a refusal, but a planned epic should graph clean. (Checkpoint boundaries and mid-run escalations are structural, not planning products, and are not linted.)

**A configured operator channel changes the economics of the survivors, not the triage.** When the run can park a question for the human (`tk ask` / `tk answer` — SKILL.md → *Assisting with Awaiting Ticks*) and the runner can reach them on a device, a post-work approval is cheaper than a return to the terminal. What does not change is anything above: a question whose answer the work won't change is still resolved during planning, "do now" tasks are still done during planning, and a cheap interrupt is still an interrupt. The rules below are unchanged too — ask early, block late, split around the decision. Reachability is a latency fix, never a licence to defer decisions into the run.
