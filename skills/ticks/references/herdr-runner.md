...
You do not have to register yourself for the guard: the first `tk herd spawn` arms it on your own pane (see [*The orchestrator is an agent too*](#the-orchestrator-is-an-agent-too)) — the one agent in the run nothing else supervises is you, and arming that by hand is what everyone forgot. `tk herd watch --status` confirms it.
...
parks an `awaiting escalation` question on a blocked recorded worker's tick,
or an agent-scoped question for an explicitly watched non-tick target such as
the orchestrator pane. It holds channel delivery for the supplied grace
period, then sends the first answer back through `agent.prompt` so this wait
can resume the same target. A local worker answer uses `tk answer <tick-id>
<answer>`; an orchestrator/pane answer uses the printed `tk answer
<question-id> <answer>`. Without the flag, `blocked` remains a settled
nonzero outcome. Recorded manifests still provide exact worker/respawn
identity, while an explicitly watched non-worker target is relayed as an
agent-scoped question rather than silently dropped.
...
- **Optional terminal-first relay.** When the wave is run with `--relay-blocked-after <duration>`, the blocked target's question is parked locally first. `tk answer <tick-id> <answer>` answers a worker question, while `tk answer <question-id> <answer>` answers an agent-scoped orchestrator question. A terminal answer or direct handling of the pane wins during the grace period; only an unanswered question reaches the operator channel, and the response is prompted back to the exact target. A missing channel leaves the question terminal-only.
...
- **Provision the guard's trigger at orchestration start: `tk herd plugin --install`.** The watchdog is armed by `tk` but *invoked* by the herdr-ticks plugin's `pane.agent_status_changed` hook, which is the one environment dependency in the anti-stall path — and it failed silently for the entire life of the feature. `tk herd plugin` checks for the **hook**, never for a version: the plugin reported `0.1.0` both before and after the guard landed, so an install that was present, enabled, and three weeks stale passed every presence check while the guard never ran once. `--install` is idempotent and also updates a stale install; `--check` exits 10 when the guard cannot fire. The first `tk herd spawn` of a run warns when it arms a watch the plugin cannot serve, so a missed run-start step is loud rather than silent.
- **Arming is automatic — the first `tk herd spawn` of a run does it.** Spawn already runs in the orchestrator's own pane, so `HERDR_PANE_ID` names the very process dispatching the worker, and spawn registers it the same way it writes each worker's manifest. This is deliberate: for its first months the watchdog was armed only by a hand-typed `tk herd watch` that nothing in the codebase ever ran, so the guard file-tested for a registration that was never written and silently did nothing on **every** run. A worker is supervised as a side effect of being dispatched; the orchestrator now is too. `tk herd watch --status` confirms the registration, a bare `tk herd watch` arms this pane by hand, and `tk herd watch <agent-or-pane>` still registers an explicit target — which always wins, and is never overwritten by a later spawn (re-arming would reset the guard's episode memory mid-stall). With the herdr-ticks plugin installed, its guard hook then judges the registered target on every `pane.agent_status_changed`, exactly like the notify hook judges workers: **idle with an actionable frontier** (`tk frontier --check` exits 0) → the guard `agent.prompt`s a nudge restating the continuation rules — bounded per stall episode (`--nudge-max`, default 3, with a `--nudge-interval` floor), then one `request` chime and silence; **idle at rest** → nothing, an idle orchestrator waiting on humans is legitimate; **blocked** → one `request` chime — the guard never drives an approval UI, for the orchestrator exactly as for workers; **working** → the episode memory re-arms. `tk herd guard` is also runnable by hand for a one-off judgment, and `tk herd watch --status`/`--clear` manage the registration (state: `.tick/logs/herd/.watch-orchestrator.json`). **The guard judges the run's scope, not the repository:** the watch records the epic of the first tick spawn dispatched (or `tk herd watch --scope <id>`, an epic or a project), and the frontier it consults is that container's — descendants at any depth — so a repository full of unrelated open ticks does not read as a stalled run; the nudge names the scope (`the frontier of 692 is actionable: …`). **`--clear` is a tombstone, not a removal:** a later spawn will not re-arm a watch the operator switched off; only an explicit `tk herd watch <target>` replaces it. Field-observed 2026-09-02 before both: six nudges in one run about ticks in other epics, and a cleared watch re-armed by the next wave's spawn (tick t62).
...
- **The review phase is INVISIBLE to the operator.** Review is deliberately not a helper capability (see [Capability mapping](#capability-mapping)), so a reviewer is a harness subagent at the controller checkout — it appears in no pane, in `herdr agent list`, on the board, or in the badges. An operator watching a run therefore sees the implementers go quiet and then nothing at all for the length of a frontier review over a whole epic diff, with no signal that anything is happening; field-observed 2026-08-14, the user asked why the reviewer was not in herdr. Two consequences worth stating rather than fixing silently: the orchestrator should *say* when it dispatches a review and roughly what it covers, since the board cannot; and unlike a worker, a reviewer does **not** survive an orchestrator crash — the review tick simply stays open and is re-reviewed, which is safe because the pass is read-only. If herdr later grows a non-worktree agent spawn (an agent in a pane at the controller checkout, no branch of its own), review becomes expressible here and this limitation goes away.
