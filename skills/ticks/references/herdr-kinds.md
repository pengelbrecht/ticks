...
**In a tick run you do not type these calls: `ticfac run-epic`'s spawner does, and it also handles the startup races and the content gate that the raw sequence leaves to you** (the old `tk herd spawn` implemented this in-repo; that command is gone and the loop moved to ticfac — see [`herdr-runner.md`](herdr-runner.md#the-wave-execution-loop-moved-to-ticfac)). The contract below is what a spawner for this substrate has to implement, and what you need when *verifying a new kind* — the loop in [Adding a kind](#adding-a-kind) is exactly this, run by hand in a scratch pane.
...
The spawner under this substrate is `ticfac run-epic`'s (formerly `tk herd spawn`, deleted from this repo and moved to ticfac): the compilation below, its fixed argv order, and the fail-closed refusal further down are the contract it implements, and the argv herdr echoed on `agent_started` is recorded in run state as the ground truth for what actually landed.
...
The old `tk herd spawn` enforced this, and enforced it **before it dialled herdr** — verified live against herdr 0.8.0, 2026-08: the refusal below exited 1 having made zero herdr calls, so no workspace, worktree, branch or manifest existed to clean up afterwards. Dated example, from a command that no longer exists in this repo; whatever spawner ticfac runs now owes the same fail-closed property, and re-verify against its own build rather than assuming this one still holds.
...
`<git-common-dir>` is `git rev-parse --git-common-dir` resolved from the repo root — never a hardcoded `.git`, because a repo may keep it elsewhere. It is required whenever the worker runs in a **linked worktree**, which is every worker ticfac's spawner starts under this substrate; see *Full auto* below.
...
- **Linked worktrees need no grant.** Unlike codex, opencode has no sandbox to widen: with `--auto` a worker in a linked worktree commits normally. Verified end to end 2026-08-13 — a real `tk herd spawn` worker (the command this repo has since deleted; the equivalent check now belongs to ticfac's spawner) created a file, committed it on `tick/<id>` from `~/.herdr/worktrees/<repo>/tick-<id>`, and the collect check reported `ready-to-merge`. The `opencode` kind therefore declares **no computed per-spawn extras**.
...
Two consequences, both a spawner for this substrate has to implement (the old `tk herd spawn` did, before the wave loop moved to ticfac): the kind's family check is **strict** — provider-qualified, no `:` — so the whole class is refused before a pane exists; and when verifying a kind by hand, **read the pane footer for the model name**, not just the answer.
...
**Rule: gate a worker on the content of its first round-trip, not on its lifecycle status.** Send a trivial prompt whose answer you can pattern-match (`Reply with the single word OK`) and read the pane for the answer before handing it a tick. This is cheap and it catches the whole class of model-name, auth, and quota failures that otherwise surface as a silently empty implementer report an hour later. The old `tk herd spawn` performed this gate on every worker and failed the spawn with a pane excerpt when it did not pass; ticfac's spawner owes the same gate now that the wave loop lives there — so under a compliant spawner the trap is closed by construction. The rule still matters when you are verifying a kind by hand.
