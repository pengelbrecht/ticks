...
**Actor convention.** Export `TK_ACTOR=orchestrator` (or `TK_ACTOR=agent:<name>`) at run start so that every `tk` command from this session stamps activity entries with a recognisable actor. When the `--actor` flag is available it overrides `TK_ACTOR` for a single call; precedence is `--actor` flag > `TK_ACTOR` env > tick-owner default. This keeps orchestrator actions distinguishable from human actions in the activity feed without any per-command overhead.
