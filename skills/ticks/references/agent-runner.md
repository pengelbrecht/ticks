...
0. Read the run config (.tick/runners.toml + .tick/config.md, both optional) — run
   ...
   Under the herdr substrate, wave dispatch, wait, collect, cleanup, reconcile and the
   orchestrator watchdog are `ticfac run-epic <epic-id>`'s job now, not a `tk herd` ritual —
   see herdr-runner.md for what `tk herd` still covers (`dashboard`, `relay`) and what moved.
...
3. For each wave:
   ...
   f. Only after that gate passes, close all wave ticks durably, then clean their worktrees/branches.
      ...
      Never use a bare `tk close`. Each close prints a `frontier:` verdict — read it; on an epic
      or closeout tick it says outright whether the turn may end. Then re-read run-charter.md (this skill's references/) — fresh
...
**Run continuously.** Once the user has asked you to execute the epic, work wave to wave without stopping to ask "should I continue?". The only reasons to stop are: a blocker you can't resolve, genuine ambiguity that prevents progress, or the epic is done. Progress-summary check-ins between waves just cost the user time. The rule has a mechanical check: `tk frontier --check` exits 0 while dispatchable work exists and 1 when the run is legitimately at rest — before ending a turn you believe is a stopping point, run it; an exit 0 means the turn is not over (`tk frontier` says what is actionable).

**You will also be told without asking.** `tk close` evaluates the same predicate and prints the verdict on the close itself: one quiet `frontier:` line on an ordinary tick, and on an **epic** or a **closeout-role** tick a loud `THIS IS NOT A STOPPING POINT` naming what is dispatchable — or `frontier: at rest` when stopping is legitimate. This is the countermeasure that needs no environment: it rides a command you are already running, so unlike a harness hook or a herdr plugin there is nothing to install, configure or keep current. Read it; it is addressed to you. Harnesses and substrates may wire the same predicate into their own turn-end mechanism as well (under herdr, the guard hook does — see `herdr-runner.md`).
...
- `tk herd relay --agent <name> --grace <duration>` applies the same terminal-first grace window to one blocked herdr worker: it parks a durable question (tick-scoped when the agent matches a recorded worker manifest, otherwise an explicit agent-scoped relay), waits, and sends the answer back with herdr's `agent.prompt` so the worker resumes. Under the herdr substrate, `ticfac run-epic` is what drives this per blocked worker during a wave; a human or a fresh session can also run it by hand against a still-blocked agent. Without it, a blocked worker stays a settled nonzero outcome that the wave reports rather than relays.
