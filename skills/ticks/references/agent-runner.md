...
```
0. Read .tick/config.md (if present) — run its `At run start` (legacy: Environment) pre-flight
   checks now, before wave 1.
...
- **`For implementers`** (legacy alias: `Rules`) — project-specific constraints to include verbatim in every implementer prompt.
