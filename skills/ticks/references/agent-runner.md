...
3. **Say what you resolved.** An override is a deliberate choice, not a degradation, but a substrate nobody announced is one nobody can audit afterwards: state it in your own output before dispatching the first worker and record it durably — `tk note <epic-id> "runner-state: substrate=harness requested=harness config=herdr source=TICKS_SUBSTRATE reason=explicit-override"`. (`tk sandbox substrate` resolves and prints both lines for you.)
4. **`substrate = "herdr"` with herdr unavailable degrades explicitly — it never fails the run.** Before dispatching the first worker, state the requested substrate, the probe(s) that failed, and the harness adapter you are falling back to; record the same fact durably (`tk note <epic-id> "runner-state: substrate=harness requested=herdr reason=herdr-unavailable"`); then continue under harness dispatch.
