...
func TestResolveOperatorMergesPublicMappingWithGlobalCredentials(t *testing.T) {
	repo := t.TempDir()
	mapping := Mapping{}
	mapping.UpsertOperator("alice", "telegram", OperatorIdentity{BotUsername: "alice_bot", UserID: 42})
	if err := SaveMapping(repo, mapping); err != nil {
		t.Fatalf("SaveMapping() error: %v", err)
	}

	t.Setenv(HomeEnv, filepath.Join(t.TempDir(), "ticks"))
	global := OperatorConfig{}
	global.SetChannel("telegram", ChannelConfig{
		Token:       "123:secret",
		ChatID:      "9001",
		UserID:      "global-user-must-not-win",
		BotUsername: "global-bot-must-not-win",
		APIBase:     "http://fake",
	})
	if err := SaveOperatorConfig(global); err != nil {
		t.Fatalf("SaveOperatorConfig() error: %v", err)
	}

	resolved, err := ResolveOperator(repo, "alice")
	if err != nil {
		t.Fatalf("ResolveOperator() error: %v", err)
	}
	telegram, ok := resolved.Channel("telegram")
	if !ok {
		t.Fatal("resolved operator has no telegram channel")
	}
	if telegram.Token != "123:secret" || telegram.ChatID != "9001" || telegram.APIBase != "http://fake" {
		t.Fatalf("global credentials/config not preserved: %#v", telegram)
	}
	if telegram.BotUsername != "alice_bot" || telegram.UserID != "42" {
		t.Fatalf("public mapping did not override global identity: %#v", telegram)
	}
}
