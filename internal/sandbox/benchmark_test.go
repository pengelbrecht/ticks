...
// The dispatch layer's probe budget against the committed benchmark (tick
// 7go: the budget was 30s while a cold container measurably takes 93.24s, so
// the first real per-tick wave wrote off three healthy containers 21 seconds
// after dispatching them) used to be pinned here by reading the shipped
// constant straight out of cloud/factory/src/worker-dispatch.ts. That file
// left this repo for ticfac (tick 3r2); the guard comparing it against the
// committed sandbox-start benchmark is ticfac's to keep now that both halves
// of the comparison — the TypeScript constant and the container it bounds —
// no longer live in the same repo.
