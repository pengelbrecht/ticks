...
func (c *Client) Call(ctx context.Context, method string, params any, out any) error {
	...
	if err := json.Unmarshal(raw, out); err != nil {
		return fmt.Errorf("herdclient: decoding %s result: %w", method, err)
	}
	return nil
}
...
func (c *Client) callRaw(ctx context.Context, method string, params any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("herdclient: reading %s response: %w", method, err)
	}
...
func (c *Client) send(ctx context.Context, method string, params any) (Conn, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("herdclient: encoding %s request: %w", method, err)
	}
	...
	if err := conn.WriteMessage(ctx, payload); err != nil {
		conn.Close()
		return nil, "", fmt.Errorf("herdclient: sending %s request: %w", method, err)
	}
	return conn, id, nil
}
...
func decodeResponse(method, wantID string, line []byte) (json.RawMessage, error) {
	...
	if err := json.Unmarshal(line, &resp); err != nil {
		return nil, fmt.Errorf("herdclient: malformed %s response: %w", method, err)
	}
	if resp.Error != nil {
		return nil, &APIError{
			Method:    method,
	...
	if len(resp.Result) == 0 {
		return nil, fmt.Errorf("herdclient: %s response has neither result nor error", method)
	}
	if resp.ID != wantID {
		return nil, fmt.Errorf("herdclient: %s response id %q does not match request id %q", method, resp.ID, wantID)
	}
