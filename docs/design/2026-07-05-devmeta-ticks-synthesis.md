...
**Refinement (2026-07-07, post-campaign): config is METHOD, never per-increment STATE.** The benchmark-run configs hardcoded the increment path, tag prefix, and brief pointer — a state-in-method smell (editing your methods file every increment). Fixed convention: config's lifecycle sections *resolve* the active increment from `.devmeta/current-increment.md` (already maintained per increment); tags derive from the increment id (`<id>.<epic-slug>`); the spec/brief pointer lives in the epic descriptions (already the roadmap convention — implementer prompts inline the tick description, so the config copy was redundant). Config is then written once per project and edited only on genuine method changes — the same state/method separation the synthesis enforces everywhere else (live state in tk, narrative records write-once, parameters in the profile).
...
**DECISION (2026-07-07): drafted, deliberately NOT implemented.** On reflection the full mechanism fails the campaign's own evidence-first bar: the sole data point (run 4's 50-min tail) produced real fixes and the series' most polished output — arguably the system working, not failing; the asymmetry argues the current always-frontier default IS the safety-biased policy; the yield-prior would be noise at 3-epics-per-project scale; and the machinery-per-benefit is the worst of any proposal (±30 min stakes on multi-hour runs, against real skill-bloat cost). **Adopted: only the ledger line** — the retro records review depth, time spent, and findings-by-severity (pure observability, near-zero cost). Revisit the full P9 only if accumulated real-project ledgers show systematic waste. The rest stays drafted as the design to reach for *if* the evidence ever arrives.
...
**Post-verdict forensic (settles the re-benchmark question):** commit-level comparison refutes the "doctrine inflated the cuts" theory — run 4's E3 wave-2 ticks were *smaller* than run 3's (348/247/255 insertions vs 385/651/194), and its E3 implementation was the **fastest of the series** (plan-close 02:15 → all three merged 02:56, ~41 min vs run 3's ~60). The entire excess sits in the **post-integration tail** (02:56→03:47: frontier review that found and fixed issues 2-wide, retro, checkpoint report — and the series' most polished output, e.g. Prettier fully green). The regression is a review-depth/rigor-policy variance, not a P8 cost. **Re-benchmarking P8 would sample review-tail variance, not doctrine — not worth the runs.** If the time question ever needs settling: interleaved same-day P7/P8 pairs, n≥3 per config.
...
**Ablate-now decision (2026-07-07): no new ablation run on current models.** (a) The P8 ablation already exists — run 3 (no P8 text) vs run 4 (with) measured delta ≈ 0 on partition quality, within-noise on time → P8's procedures are mildly redundant on current models; compression is licensed by existing data. (b) A fresh happy-path run cannot test the remaining procedural mass, which exists for unhappy paths (recovery, conflicts, drift) a clean run never samples. (c) Quality metrics are at ceiling (0/0) and single-run time has no power (±25% noise, per §13). **Adopted path instead: write the principles-only "constitution" variant of the skill now, on a branch beside the full version; run the next REAL project on it with ledgers recording (real work = continuous ablation, unhappy paths included, fallback one cp away). Reserve the formal synthetic ablation pair for the next model generation, on a fresh brief.**
