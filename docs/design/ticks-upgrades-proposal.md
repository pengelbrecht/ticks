# Proposed Ticks Upgrades — Synthesis Outcomes

*Fork: `mkelk/ticks-melk` (default branch `2026-07-05-synthesis`). The items below are
implemented on this fork and exercised across four runs of one full-stack brief. Analysis,
per-run results, and forensics:
[`2026-07-05-devmeta-ticks-synthesis.md`](2026-07-05-devmeta-ticks-synthesis.md) (§9–§14).*
...
- **Part 1 — stand-alone engine changes (P6, P7, P8).** They apply to any ticks project with no
  ...
  runs 2–4: multi-wide parallel execution, no merge conflicts, no mid-run interventions.
- **Part 2 — one adaptation seam (P3).** Lifecycle-addressed `.tick/config.md` sections let a
  project inject steps at boundaries the engine already owns, without forking. `dmtix`
  (`skills/dmtix/` in [`mkelk/skills`](https://github.com/mkelk/skills)) is an example method
  built on this seam.

All changes are skill-prose only (no `tk` binary change) except two optional CLI conveniences
noted inline. A further proposal (P9) was drafted and not adopted; its reasoning is recorded
below.
...
### P6 — Execution readiness: the project profile
...
**Mechanism.** A characterization step (loop step 0.5) writes and maintains `.tick/profile.md`:
an inferred **provisioning recipe** (what makes a worktree runnable, validated by a solo probe
before wave 1), a **tier→venue map** (each test tier runs *in-worktree* when it is provably
isolated, else *post-merge, serial* on a merge candidate), and required services. The routing
is conservative: a tier is treated as parallel-safe only on positive evidence of isolation,
since a wrong "parallel" corrupts silently while a wrong "serial" only costs time. Post-merge
verification uses candidate merges (`git merge --no-commit` → verify → commit or abort) so the
integration branch is not left broken and a failure attributes to the merged tick.

**Observed.** Runs 2–4 provisioned runnable worktrees and executed waves in parallel; the
profile was derived from an empty project config each run.

**Touches.** `SKILL.md` Step 0 · `agent-runner.md` (profile, provisioning, venues, loop steps
0.5/3f, retro re-characterisation).

### P7 — Dispatch economics: warm-chains
...
worker — a warm sequential engine matched the parallel one on wall-clock in the calibration
run.
...
**Observed.** The gate selected all three modes across one run where each was cheapest, with
the arithmetic recorded per wave.

**Touches.** `agent-runner.md` (dispatch modes, gate, chain prompt variant, retro ledger) ·
`claude-runner.md` (a chain maps to one agent).

### P8 — Constraint-surface partitioning
...
**Observed.** Run 4 produced this partition from an empty state with the rationale citing the
rules, and integrated with no merge conflicts; the seam that had produced the one conflict in
run 2 was placed in a single tick.
...
severity. It changes no behaviour (the always-review default stands); it records review
cost and yield as data. It is the only part of the drafted P9 that was kept.

---

## Part 2 — Adaptation seam

### P3 — Lifecycle-addressed config sections
...
| `At run start` (alias `Environment`) | once, before wave 1 | 1× |
...
**Config as method, not state.** These sections resolve per-increment facts (an increment id,
a report directory) from the project's own files rather than embedding them, so `config.md` is
...
**Example — dmtix.** A delivery method (interactive increment scoping, an autonomous run, and
per-increment narrative records with checkpoints) implemented on these sections plus roadmap
conventions, with no change to the engine. It is packaged as three argument-routed entry points
over the unchanged ticks skill and lives in
[`mkelk/skills` → `skills/dmtix/`](https://github.com/mkelk/skills). Its record-keeping runs
under `At epic close-out` and `At project checkpoint`; the engine is unaware of it.
...
## Not adopted: P9 (review-depth economics)
...
failure asymmetry is the reverse of P7's: skipping a needed review has open-ended cost, while
running an unneeded one costs bounded time. On that asymmetry the current always-review default
is already the conservative choice, and machinery whose main effect is to permit less review is
hard to justify from one data point. Only the observability ledger (Part 1) was kept; the rest
is left for real-project data.
...
| Live state | `tk` |
| A method's narrative records | the method's own tree (dmtix → `.devmeta/`) |
| `tk` binary | unchanged (two optional conveniences noted inline) |

P3 and P6 are complementary: P6 supplies waves with runnable inputs; P3 lets boundaries emit
declared outputs. Part 1 addresses parallel execution on real repositories; Part 2 defines a
seam for methods so they need not fork the engine.
