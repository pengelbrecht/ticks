...
| D22 | Ignition on a leased project refuses with the holder's run ID; `--queue` is the opt-in park-and-ignite-on-release, visible to `status` and expiring on a configurable window. An expiry is **announced, never silent** (tick 6tx): a PR review that expires unrun comments on the pull request saying so and why, because "no review because nothing was wrong" and "no review because the window closed behind an epic run" must not be one observation | UC1b, UC5 |
