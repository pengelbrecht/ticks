...
- no fresh checkout-wide controller lease or ambiguous duplicate branch/worktree/manifest claim. The lease is keyed by the canonical checkout root (not epic ID) and is shared by `/ticks-run --execute`, `/ticks-plan --apply`, and dashboard tracker actions across separate Pi processes, so none can share one Git index/tracker authority; manifests and telemetry remain operation-specific.
...
Real `tk graph --json` omits awaiting-human children from `waves` while retaining them in `stats.awaiting_human`. The runner treats that statistic as authoritative, resolves ordinary gates with `tk next <epic> --awaiting= --json`, and returns `status: awaiting` with tick ID, title, gate type, and operator action. In autonomous mode, `tk next <epic> --autonomous --json` remains policy authority; when it selects an omitted checkpoint child, the runner retrieves `tk graph <epic> --all --json` and makes only that exact open, role-matching checkpoint ready. Arbitrary task injection, parent/epic mismatch, and status/role disagreement fail closed. An awaiting-only epic is never reported completed.
...
- Package proof: `node --no-warnings scripts/verify-pi-ticks-qfs.ts package-rpc`
- Live proof: `node --no-warnings scripts/verify-pi-ticks-qfs.ts live-scenario`

## Acceptance Evidence
- A1: `node --test extensions/ticks-runner/*.test.ts`
- A2: `node --no-warnings scripts/verify-pi-ticks-qfs.ts live-scenario` — trailing audit prose is allowed
...
Prose outside the inline-code span is never sent to a shell. A prose-only Environment bullet blocks execution; a prose-only Testing bullet remains a child prompt hint but is not executed; malformed Closeout Evidence Commands fail closeout authorization and are never executable. Acceptance criteria and Rules are always prose, even when they contain backticks; they never authorize shell execution. `Acceptance Evidence` is a strict bounded authorization table: `- A<n>: \`exact command\``. Every current epic item has exactly one mapping. Its command must match exactly one executable entry across Testing plus Closeout Evidence Commands; duplicate item mappings, duplicated/ambiguous commands across either section, unknown items, multiple code spans, and injected text fail closed. The same unique command may map different items only through separate exact A<n> lines. Commands run through `/bin/sh -lc` on POSIX or `cmd.exe /d /s /c` on Windows and stop at the first failure.
...
Branches are `tick/<epic>/<tick>`. Manifests are atomically written and carry no PID or Pi session ID. Event logs may be large: status lists them but never loads them. Recovery reads reports with byte/file/count limits. Each run also writes a bounded `dashboard-history.json` containing the latest normalized model, cumulative task/dependency-to-stage projection, and a short status history; later status/dashboard reconstruction restores stable completed/current/upcoming stages plus agent, verification, merge, and usage lanes without reading event logs.
