 ...
 *
 * The extension ships with no runtime dependencies and Node has no TOML
 * parser, so this covers exactly the syntax `runners-config.schema.json`
 * describes: tables, dotted keys, strings (basic, literal and multi-line),
 * integers, booleans, arrays and inline tables. Anything else — array of
 * tables, dates, floats with exotic forms — is refused by name rather than
 * silently skipped: this file decides which shell a run may execute, so an
