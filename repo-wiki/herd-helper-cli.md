...
## Epic zz0 additions (in progress)
...
Exit codes: `GetExitCode` is now purely typed (`errors.As` on `ExitError`; substring heuristics removed in epic zz0 — usage errors are converted at the source via `SetFlagErrorFunc` + a tree-wide `usageArgs` wrapper, so new commands are correct by construction). Two observable changes recorded in CHANGELOG: EINVAL-shaped runtime errors 2→1; `NoArgs` arity mistakes 1→2. Known residual inconsistency (retro follow-up): ~15 commands wrap repoRoot/tick-not-found failures as exit 1 instead of ExitNoRepo(3)/ExitNotFound(4). The wait TestWaitTimeout contention flake is documented as environmental in `.tick/config.md` Testing. Smoke: `bash scripts/verify-herd-helper.sh [--quick]` (also a Closeout Evidence Command in `.tick/config.md`); full mode includes the kill-9 reconcile drill. Field report: `docs/design/herd-helper-smoke-report.md`.
