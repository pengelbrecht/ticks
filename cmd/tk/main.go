...
func run(args []string) int {
	...
	switch args[1] {
	case "init", "whoami", "show", "create", "new", "update", "close", "reopen", "delete", "block", "unblock", "note", "notes", "list", "ls", "ready", "next", "blocked", "label", "labels", "deps", "graph", "roadmap", "context", "status", "rebuild", "merge-file", "stats", "view", "snippet", "import", "approve", "reject", "version", "upgrade", "migrate", "gc", "merge", "board":
...
func printUsage() {
	...
	fmt.Println("Commands: init, whoami, show, create (new), block, unblock, update, close, reopen, note, notes, list (ls), ready, next, blocked, rebuild, delete, label, labels, deps, graph, roadmap, context, status, merge-file, stats, view, snippet, import, approve, reject, board, version, upgrade, migrate, gc, merge")
	fmt.Println()
