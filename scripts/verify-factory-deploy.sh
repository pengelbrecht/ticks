...
# produces a working authenticated factory endpoint and a ~/.ticfacrc entry, and
...
#                            secret pushed, endpoint verified, ~/.ticfacrc holds
...
TICFACRC="$HOME/.ticfacrc"
count_calls() { grep -c -- "$1" "$FAKE_WRANGLER_LOG" || true; }
rc_value() { sed -n "s/^$1=//p" "$TICFACRC"; }
...
[ ! -e "$TICFACRC" ] || fail "a failed precondition wrote $TICFACRC"
...
grep -q "^factory_token=tkf_" "$NPX_HOME/.ticfacrc" || fail "no token was recorded on the npx path"
...
[ -f "$TICFACRC" ] || fail "$TICFACRC was not written"
...
if [ "$(uname)" != "Darwin" ]; then
  PERM="$(stat -c '%a' "$TICFACRC")"
else
  PERM="$(stat -f '%A' "$TICFACRC")"
fi
[ "$PERM" = "600" ] || fail "$TICFACRC is mode $PERM, want 600"
...
pass "endpoint verified, credentials in $TICFACRC (mode 600), resources created once"
